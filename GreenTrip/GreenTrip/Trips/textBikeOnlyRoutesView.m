...
@synthesize busRoute,bikeRoute,walkingRoute,startName,endName,allRoutesDictionary,busName,startBikeStopName,endBikeStopName,flag;
...
- (void)getRoutesDetails:(NSMutableDictionary *)dic
{
    ...
    while ((key = [enumerator nextObject])) {
        ...
        NSLog(@"%@",key);
        if ([key isEqualToString:@"startWalking"]) {
            ...
            [strategyArray insertObject:[NSString stringWithFormat:@"步行%@",[self distanceFormatted:walkingDis]] atIndex:0];
            [detailWaysArray insertObject:walkArray atIndex:0];
            [strDetailsArray addObject:@""];
            [flagArray insertObject:@"0" atIndex:0];
            
            
        } else if ([key isEqualToString:@"endWalking"]) {
    ...
    //NSLog(@"strategyArray =  %@",strategyArray);
...
- (void)creatBikeCell:(UITableViewCell *)cell inTheRow:(NSInteger)row
{
    ...
    UIButton *titleButton = [[UIButton alloc] initWithFrame:CGRectMake(52, 0, [self superview].bounds.size.width - 52, 30)];
    //titleButton.backgroundColor = [UIColor groupTableViewBackgroundColor];
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 10, [self superview].bounds.size.width - 52, 30)];
    titleLabel.text = [stopArray objectAtIndex:row];
    //titleLabel.textAlignment = NSTextAlignmentLeft;
    [titleButton addSubview:titleLabel];
    [cell addSubview:titleButton];
